...
public class AddToCartController extends AbstractController
{
	...
	public String addToCart(@RequestParam("productCodePost") final String code,
			@RequestParam(value = "parentEquipmentCode", required = false) final String parentCode, final Model model,
		...
		else
		{
			try
			{
				...
				if (null != parentCode)
				{
					...
					final List<AbstractOrderEntryModel> cartEntries = cartService.getSessionCart().getEntries();
					final AbstractOrderEntryModel lastEntry = cartEntries.get(cartEntries.size() - 1);
					lastEntry.setParentEquipment(parentCode);
					modelService.saveAll();
					model.addAttribute("parentEquipment", parentCode);
				}
